...
if [[ ${PV} = 9999* ]]; then
	...
	if [[ -n "${patchset}" ]]; then
		SRC_URI="${SRC_URI}
			http://dev.gentoo.org/~ssuominen/${MY_P}-patches-${patchset}.tar.xz
			http://dev.gentoo.org/~williamh/dist/${MY_P}-patches-${patchset}.tar.xz"
	fi
	KEYWORDS=""
fi
...
multilib_src_configure() {
	...
	if multilib_is_native_abi; then
		econf_args+=( "--with-rootlibdir=/$(get_libdir)" )
	else
		econf_args+=( "MOUNT_CFLAGS=' ' MOUNT_LIBS=' '" )
	fi

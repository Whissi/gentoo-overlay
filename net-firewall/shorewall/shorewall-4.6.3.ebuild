...
MY_P=${PN}-${MY_PV}
MY_P_IPV4=shorewall-${MY_PV}
MY_P_IPV6=shorewall6-${MY_PV}
MY_P_LITE4=shorewall-lite-${MY_PV}
MY_P_LITE6=shorewall6-lite-${MY_PV}
MY_P_INIT=shorewall-init-${MY_PV}
MY_P_CORE=shorewall-core-${MY_PV}
...
SRC_URI="
	http://www.shorewall.net/pub/shorewall/${MY_URL_PREFIX}${MY_MAJOR_RELEASE_NUMBER}/shorewall-${MY_MAJORMINOR_RELEASE_NUMBER}/shorewall-core-${MY_PV}.tar.bz2
	ipv4? ( http://www.shorewall.net/pub/shorewall/${MY_URL_PREFIX}${MY_MAJOR_RELEASE_NUMBER}/shorewall-${MY_MAJORMINOR_RELEASE_NUMBER}/shorewall-${MY_PV}.tar.bz2 )
	ipv6? ( http://www.shorewall.net/pub/shorewall/${MY_URL_PREFIX}${MY_MAJOR_RELEASE_NUMBER}/shorewall-${MY_MAJORMINOR_RELEASE_NUMBER}/shorewall6-${MY_PV}.tar.bz2 )
	lite4? ( http://www.shorewall.net/pub/shorewall/${MY_URL_PREFIX}${MY_MAJOR_RELEASE_NUMBER}/shorewall-${MY_MAJORMINOR_RELEASE_NUMBER}/shorewall-lite-${MY_PV}.tar.bz2 )
	lite6? ( http://www.shorewall.net/pub/shorewall/${MY_URL_PREFIX}${MY_MAJOR_RELEASE_NUMBER}/shorewall-${MY_MAJORMINOR_RELEASE_NUMBER}/shorewall6-lite-${MY_PV}.tar.bz2 )
	init? ( http://www.shorewall.net/pub/shorewall/${MY_URL_PREFIX}${MY_MAJOR_RELEASE_NUMBER}/shorewall-${MY_MAJORMINOR_RELEASE_NUMBER}/shorewall-init-${MY_PV}.tar.bz2 )
	doc? ( http://www.shorewall.net/pub/shorewall/${MY_URL_PREFIX}${MY_MAJOR_RELEASE_NUMBER}/shorewall-${MY_MAJORMINOR_RELEASE_NUMBER}/${MY_P_DOCS}.tar.bz2 )
"
...
# - Installing Shorewall-init or just the documentation doesn't any sense,
...
src_prepare() {
	...
	ln -s ../shorewallrc.gentoo ${MY_P_CORE}/shorewallrc.gentoo
	eend 0

	# shorewall
	if use ipv4; then
		ebegin "Applying Gentoo-specific changes to ${MY_P_IPV4}"
		ln -s ../shorewallrc.gentoo ${MY_P_IPV4}/shorewallrc.gentoo
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall.confd "${S}"/${MY_P_IPV4}/default.gentoo || die "Copying shorewall.confd failed"
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall.initd "${S}"/${MY_P_IPV4}/init.gentoo.sh || die "Copying shorewall.initd failed"
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall.systemd "${S}"/${MY_P_IPV4}/gentoo.service || die "Copying shorewall.systemd failed"
		eend 0
	fi
	...
	if use ipv6; then
		...
		ln -s ../shorewallrc.gentoo ${MY_P_IPV6}/shorewallrc.gentoo
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall6.confd "${S}"/${MY_P_IPV6}/default.gentoo || die "Copying shorewall6.confd failed"
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall6.initd "${S}"/${MY_P_IPV6}/init.gentoo.sh || die "Copying shorewall6.initd failed"
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall6.systemd "${S}"/${MY_P_IPV6}/gentoo.service || die "Copying shorewall6.systemd failed"
		eend 0
	fi
	...
	if use lite4; then
		...
		ln -s ../shorewallrc.gentoo ${MY_P_LITE4}/shorewallrc.gentoo
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall-lite.confd "${S}"/${MY_P_LITE4}/default.gentoo || die "Copying shorewall-lite.confd failed"
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall-lite.initd "${S}"/${MY_P_LITE4}/init.gentoo.sh || die "Copying shorewall-lite.initd failed"
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall-lite.systemd "${S}"/${MY_P_LITE4}/gentoo.service || die "Copying shorewall-lite.systemd failed"
		eend 0
	fi
	...
	if use lite6; then
		...
		ln -s ../shorewallrc.gentoo ${MY_P_LITE6}/shorewallrc.gentoo
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall6-lite.confd "${S}"/${MY_P_LITE6}/default.gentoo || die "Copying shorewall6-lite.confd failed"
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall6-lite.initd "${S}"/${MY_P_LITE6}/init.gentoo.sh || die "Copying shorewall6-lite.initd failed"
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall6-lite.systemd "${S}"/${MY_P_LITE6}/gentoo.service || die "Copying shorewall6-lite.systemd failed"
		eend 0
	fi
	...
	if use init; then
		...
		ln -s ../shorewallrc.gentoo ${MY_P_INIT}/shorewallrc.gentoo
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall-init.confd "${S}"/${MY_P_INIT}/default.gentoo || die "Copying shorewall-init.confd failed"
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall-init.initd "${S}"/${MY_P_INIT}/init.gentoo.sh || die "Copying shorewall-init.initd failed"
		cp "${FILESDIR}"/${MY_MAJOR_RELEASE_NUMBER}/shorewall-init.systemd "${S}"/${MY_P_INIT}/gentoo.service || die "Copying shorewall-init.systemd failed"
...
src_install() {
	...
	DESTDIR="${D}" ${MY_P_CORE}/install.sh shorewallrc.gentoo || die "${MY_P_CORE}/install.sh failed"
	dodoc "${S}"/${MY_P_CORE}/changelog.txt "${S}"/${MY_P_CORE}/releasenotes.txt

	# shorewall
	if use ipv4; then
		einfo "Installing ${MY_P_IPV4} ..."
		keepdir /var/lib/shorewall
		DESTDIR="${D}" ${MY_P_IPV4}/install.sh shorewallrc.gentoo || die "${MY_P_IPV4}/install.sh failed"

		if use doc; then
			dodoc -r "${S}"/${MY_P_IPV4}/Samples
		fi
	fi
	...
	if use ipv6; then
		...
		DESTDIR="${D}" ${MY_P_IPV6}/install.sh shorewallrc.gentoo || die "${MY_P_IPV6}/install.sh failed"

		if use doc; then
			dodoc -r "${S}"/${MY_P_IPV6}/Samples6
		fi
	fi
	...
	if use lite4; then
		...
		DESTDIR="${D}" ${MY_P_LITE4}/install.sh shorewallrc.gentoo || die "${MY_P_LITE4}/install.sh failed"
	fi
	...
	if use lite6; then
		...
		DESTDIR="${D}" ${MY_P_LITE6}/install.sh shorewallrc.gentoo || die "${MY_P_LITE6}/install.sh failed"
	fi

	# shorewall-init
	if use init; then
		einfo "Installing ${MY_P_INIT} ..."
		DESTDIR="${D}" ${MY_P_INIT}/install.sh shorewallrc.gentoo || die "${MY_P_INIT}/install.sh failed"
		dodoc "${S}"/${MY_P_INIT}/shorewall-init.README.Gentoo.txt
...
pkg_postinst() {
	...
	if [[ -n "${REPLACING_VERSIONS}" && ${REPLACING_VERSIONS} < 4.6 ]]; then
		# This is an upgrade

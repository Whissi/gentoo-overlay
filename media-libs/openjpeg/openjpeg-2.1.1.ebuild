...
MY_TESTDATA_COMMIT="ce43949b0a815e1d9236b642ef59ea0c5a7eb522"

DESCRIPTION="An open-source JPEG 2000 library"
HOMEPAGE="https://github.com/uclouvain/openjpeg"
SRC_URI="https://github.com/uclouvain/${PN}/archive/v${PV}.tar.gz -> ${P}.tar.gz
	test? ( https://github.com/uclouvain/openjpeg-data/archive/${MY_TESTDATA_COMMIT}.tar.gz -> ${PN}-data_20160508.tar.gz )"
	...
	if ctest "${myctestargs[@]}" "$@" ; then
		...
		if [[ -f "${KNOWN_FAILURES_LIST}" && -f "${FAILEDTEST_LOG}" ]]; then
			...
			if [[ ${HAS_UNKNOWN_TEST_FAILURES} -ne 0 ]]; then
				die "New/unknown test failure found!!!"
			fi

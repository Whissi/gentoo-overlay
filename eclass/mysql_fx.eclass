...
#
# Helper function, version (integer) may not match MYSQL_VERSION_ID format
# which includes Gentoo revision numbers.
#
addrevisionpadding() {
	local strtopad=${1}
	local type=${2:-"min"}
	local lengthdiff=$(( ${#MYSQL_VERSION_ID} - ${#strtopad} ))

	if [[ ${lengthdiff} -ne 2 ]] ; then
		# Unknown format. We cannot securely add padding to match
		# MYSQL_VERSION_ID's format. Reasons:
		#
		#   a) 00000_index.txt is already aware of MYSQL_VERSION_ID format
		#   b) Invalid @ver value in 00000_index.txt
		echo ${strtopad}
	elif [[ "${type}" == "min" ]] ; then
		echo "${strtopad}00"
	else
...
stripdots() {
	...
	if [[ "${dotver/./}" != "${dotver}" ]] ; then
		...
		for i in 100000000 1000000 10000 100 1 ; do
			v=${dotver%%\.*}
...
mysql_check_version_range() {
	local lbound="${1%% to *}" ; lbound=$(stripdots "${lbound}") ; lbound=$(addrevisionpadding "${lbound}" "min")
	local rbound="${1#* to }"  ; rbound=$(stripdots "${rbound}") ; rbound=$(addrevisionpadding "${rbound}" "max")

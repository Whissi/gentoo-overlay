...
# Patch version
PATCH_SET="https://mirror.whissi.de/distfiles/percona-server/${PN}-8.0.32.24-patches-01.tar.xz"
...
IUSE="cjk cracklib debug jemalloc latin1 ldap numa pam +perl profiling
	rocksdb router selinux +server tcmalloc test"
...
src_configure() {
	...
	if use server ; then
		mycmakeargs+=(
			...
			-DWITH_PERCONA_AUTHENTICATION_LDAP=$(usex ldap system OFF)
